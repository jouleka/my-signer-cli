...
RSpec.describe 'mysigner certificate download', type: :cli do
  ...
  describe 'when not logged in' do
    ...
    it 'shows error message' do
      output = capture_stdout { cli.certificate('download', cert_id) }
  ...
  describe 'when no ID provided' do
    ...
    it 'shows usage message' do
      output = capture_stdout { cli.certificate('download') }
  ...
  describe 'successful download with default filename' do
    ...
    it 'shows downloading message' do
      output = capture_stdout { cli.certificate('download', cert_id) }
      expect(output).to include('Downloading certificate')
    end

    it 'fetches certificate details' do
      expect(client).to receive(:get).with("/api/v1/organizations/#{org_id}/certificates/#{cert_id}")
      cli.certificate('download', cert_id)
    end

    it 'downloads certificate content' do
      expect(faraday_conn).to receive(:get).with("/api/v1/organizations/#{org_id}/certificates/#{cert_id}/download")
      cli.certificate('download', cert_id)
    end
    ...
    it 'shows certificate details' do
      ...
      expect(output).to include('DISTRIBUTION')
    ...
    it 'shows file path' do
      ...
      expect(output).to include('iOS_Distribution_Certificate.cer')
    end
  ...
  describe 'successful download with custom output path' do
    ...
    let(:faraday_response) { double('response', success?: true, body: cert_content, headers: {}) }
    let(:faraday_conn) { double('connection') }
    let(:custom_path) { '/custom/path/mycert.cer' }
    ...
    it 'saves to custom path' do
      expect(File).to receive(:binwrite).with(custom_path, cert_content)
      cli.certificate('download', cert_id)
    end
  ...
  describe 'when certificate not found' do
    before do
      allow(config).to receive(:exists?).and_return(true)
      allow(config).to receive(:load)
      allow(config).to receive(:api_url).and_return(api_url)
    ...
    it 'shows error message' do
      ...
      expect(output).to include(cert_id)
    end
  ...
  describe 'when unknown action provided' do
    ...
    it 'exits with code 1' do
      expect(cli).to receive(:exit).with(1)
      cli.certificate('upload', cert_id)
    end
  end

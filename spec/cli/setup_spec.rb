...
RSpec.describe 'mysigner setup' do
  ...
  describe 'successful setup - user has everything' do
    ...
    it 'shows welcome message' do
      expect { cli.setup }.to output(/Welcome/).to_stdout
    end

    it 'asks about account' do
      expect { cli.setup }.to output(/Do you have a My Signer account/).to_stdout
    end

    it 'asks about organization' do
      expect { cli.setup }.to output(/Do you have an organization/).to_stdout
    end

    it 'shows token generation step' do
      output = capture_stdout { cli.setup }
    ...
    it 'shows success message' do
      expect { cli.setup }.to output(/Setup Complete/).to_stdout
    end

    it 'shows organization name' do
      expect { cli.setup }.to output(/Test Org/).to_stdout
    end

    it 'shows next steps' do
      expect { cli.setup }.to output(/mysigner ship testflight/).to_stdout
    end
  end
  ...
  describe 'user needs to create account' do
    ...
    context 'user confirms account created' do
      ...
      it 'shows signup guidance' do
        expect { cli.setup }.to output(/Let's create your account/).to_stdout
      end

      it 'shows signup steps' do
        expect { cli.setup }.to output(/Click 'Sign Up'/).to_stdout
      end

      it 'continues to next step' do
        expect { cli.setup }.to output(/Organization Setup/).to_stdout
      end
    end
    ...
    context 'user has not created account' do
      ...
      it 'exits early' do
        expect { cli.setup }.to output(/Come back and run/).to_stdout
      end
  ...
  describe 'user needs to create organization' do
    ...
    context 'user confirms organization created' do
      ...
      it 'shows organization guidance' do
        expect { cli.setup }.to output(/Let's create your organization/).to_stdout
      end

      it 'shows organization steps' do
        expect { cli.setup }.to output(/Click 'Create Organization'/).to_stdout
      end

      it 'continues to next step' do
        expect { cli.setup }.to output(/Generate API Token/).to_stdout
      end
    end
  ...
  describe 'user does not have token yet' do
    ...
    it 'shows token generation guidance' do
      expect { cli.setup }.to output(/Generate API Token/).to_stdout
      expect { cli.setup }.to output(/Click 'Create Token'/).to_stdout
    end
  ...
  describe 'error handling - empty token' do
    ...
    it 'suggests running setup again' do
      expect { cli.setup }.to output(/Run 'mysigner setup' again/).to_stdout
    end
  end
  ...
  describe 'error handling - invalid token' do
    ...
    it 'shows helpful guidance' do
      expect { cli.setup }.to output(/The token you entered is invalid/).to_stdout
      expect { cli.setup }.to output(/Make sure the token hasn't been revoked/).to_stdout
    end
  ...
  describe 'error handling - no organizations' do
    ...
    it 'shows helpful guidance' do
      expect { cli.setup }.to output(/check that your token is associated/).to_stdout
    end
  ...
  describe 'error handling - unexpected error' do
    ...
    it 'suggests running setup again' do
      expect { cli.setup }.to output(/Run 'mysigner setup' to try again/).to_stdout
    end
  ...
  describe 'help text' do
    it 'has description' do
      command = Mysigner::CLI.commands['setup']
      expect(command.description).to include('Interactive setup guide')
    end

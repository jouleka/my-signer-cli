...
RSpec.describe 'mysigner signing setup', type: :integration do
  ...
  describe 'mysigner signing setup' do
    it 'runs the wizard' do
      expect(wizard).to receive(:run!)
      
      capture_output { cli.signing('setup') }
    end
    ...
    context 'with invalid action' do
      ...
      it 'shows error for unknown action' do
        ...
        expect(output).to include('Usage: mysigner signing setup')
      end
    end
  end

  describe 'help documentation' do
    it 'documents signing setup command' do
      help_output = capture_output { cli.help('signing') }
      
      expect(help_output).to include('signing setup')
